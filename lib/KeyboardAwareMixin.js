...
const KeyboardAwareMixin = {
  ...
  updateKeyboardSpace: function (frames: Object) {
    if(frames && frames.end){
      var height = frames.end.height;
      var x = frames.end.x;
      var y = frames.end.y;
    }
    if(frames && frames.endCoordinates){
      var height = frames.endCoordinates.height;
      var x = frames.endCoordinates.screenX;
      var y = frames.endCoordinates.screenY;
    }
    let keyboardSpace: number = height + this.props.extraScrollHeight
        ...
        (isAncestor) => {
          if (isAncestor) {
            ...
            UIManager.measureInWindow(currentlyFocusedField, (x, y, width, height) => {
              if (y + height > y - this.props.extraScrollHeight - this.props.extraHeight) {
